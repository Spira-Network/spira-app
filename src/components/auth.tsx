...
function Modal({
  ...
  return (
    <DialogContent
      className={`${isSingleColumn ? "max-w-xs" : "max-w-2xl"} p-8`}
    >
      <DialogHeader>
        <DialogTitle className="text-[#346284] text-4xl font-light font-robotoslab mb-4">
          {title}
        </DialogTitle>
      </DialogHeader>
      <form
        className={`${
          isSingleColumn ? "flex flex-col" : "grid grid-cols-2"
        } gap-6`}
      >
      ...
      >
        ...
        {footerLinkText && (
          <button
            ...
            className="text-[#999] font-medium text-sm"
          >
            {footerLinkText}
          </button>
        )}
