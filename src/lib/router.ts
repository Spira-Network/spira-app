...
async function fetchHTML(
    ...
    try {
        ...
        const res = await fetch(href + '.html')
...
const updateHistoryAndScrollPosition = (toLocation: URL, replace: boolean, intraPage: boolean) => {
    ...
    if (toLocation.href !== location.href) {
        if (replace) {
            ...
            history.pushState({ index: ++currentHistoryIndex, scrollX: 0, scrollY: 0 }, '', toLocation.href)
        }
    ...
    for (const el of newDocument.querySelectorAll('head link[rel=stylesheet]')) {
        ...
        if (
            !document.querySelector(
                `[${PERSIST_ATTR}="${el.getAttribute(PERSIST_ATTR)}"], link[rel=stylesheet][href="${el.getAttribute(
                    'href',
                )}"]`,
            )
        ) {
...
async function transition(direction: Direction, toLocation: URL, options: Options, popState?: State) {
    console.log('transition', direction, toLocation, options, popState)
    ...
    if (supportsViewTransitions) {
        finished = document.startViewTransition(() => updateDOM(newDocument, toLocation, options, popState)).finished
    } else {
...
async function prepareForClientOnlyComponents(newDocument: Document, toLocation: URL) {
    ...
    if (newDocument.body.querySelector(`astro-island[client='only']`)) {
        ...
        async function hydrationDone(loadingPage: HTMLIFrameElement) {
            await new Promise(r => loadingPage.contentWindow?.addEventListener('load', r, { once: true }))
